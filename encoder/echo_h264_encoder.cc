...
static encoder_resolution_t resolution_infos[RESOLUTIONS_NUMBER] = {
	...
	{ VIDEO_720P, PIC_WIDTH_1280, PIC_HEIGHT_720, 4000, 500 },
...
bool H264Encoder::init(int frame_rate, int src_width, int src_height, int dst_width, int dst_height,int bps)
{
	if (inited_)
		return false;
    ...
    bitrate_kbps_=bps;
    if(bps<resolution_infos[curr_resolution_].min_rate){
        bitrate_kbps_=resolution_infos[curr_resolution_].min_rate;
    }
    if(bps>resolution_infos[curr_resolution_].max_rate){
...
void H264Encoder::set_bitrate(uint32_t bitrate_kbps)
{
	...
	if (bitrate_kbps_ != en_param_.rc.i_vbv_max_bitrate){
		...
		en_param_.rc.i_bitrate = bitrate_kbps_;
...
{
	en_param_.rc.i_bitrate = bitrate_kbps_;//res.min_rate;
	en_param_.rc.i_vbv_max_bitrate =bitrate_kbps_;
	en_param_.rc.i_vbv_buffer_size = 1000;
    ...
    //https://blog.csdn.net/stpeace/article/details/8287107
	en_param_.rc.i_qp_min = 10;
	en_param_.rc.i_qp_max = 51;
    en_param_.rc.i_qp_step = 4;
	en_param_.rc.i_qp_constant = 26;
	...
	en_param_.i_frame_reference = 1;  //4
	...
	en_param_.rc.f_qcompress = 1.0;//0.75;
