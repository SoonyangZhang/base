...
class RTC_LOCKABLE CriticalSection :public Lock{
 public:
  CriticalSection();
  ~CriticalSection();

  void Enter() const override;
  bool TryEnter() const override;
...
class RTC_SCOPED_LOCKABLE CritScope {
 public:
  explicit CritScope(const Lock* cs) RTC_EXCLUSIVE_LOCK_FUNCTION(cs);
  ~CritScope() RTC_UNLOCK_FUNCTION();

 private:
  const Lock* const cs_;
...
class TryCritScope {
 public:
  explicit TryCritScope(const Lock* cs);
  ~TryCritScope();
#if defined(WEBRTC_WIN)
